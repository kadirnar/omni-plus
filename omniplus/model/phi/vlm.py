...
from omniplus.model.phi import modeling_phi, modeling_siglip, projection


def build_vlm(
        image_encoder_str='vit-base-patch16-224',
        llm_str="Phi-3-mini-4k-instruct",
    ...
    image_processor, image_encoder = modeling_siglip.get_image_encoder(image_encoder_str, peft=True)

    # Language model tokenizer and llm
    #language_tokenizer, language_model = llm.get_llm("mistralai/Mistral-7B-v0.1")
    language_tokenizer, language_model = modeling_phi.get_llm(llm_str)
    ...
    vlm = VisionLanguageModel(
        image_processor, image_encoder, image_tokenizer, language_tokenizer, language_model)
...
class VisionLanguageModel(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()  # initialize self._modules as OrderedDict - enables nested nn modules
        ...
        self.start_vec = self.embed_ints(torch.tensor(
            self.language_tokenizer(imstart_str)['input_ids'])).unsqueeze(0)
        self.end_vec = self.embed_ints(torch.tensor(
            self.language_tokenizer(imend_str)['input_ids'])).unsqueeze(0)
        self.query_vec = self.embed_ints(torch.tensor(
            self.language_tokenizer(language_prompt)['input_ids'])).unsqueeze(0)
    ...
    def forward(self, batch):
        """Given a batch, format the input, do the forward, get the logits, return logits, loss Predicts the
        next token given an image, random substring of caption.
        """
        ...
        llm_input = torch.cat((self.start_vec, tokenized_image, self.end_vec, self.query_vec, caption_prefix),
                              dim=1)  #.permute(0,2,1)
